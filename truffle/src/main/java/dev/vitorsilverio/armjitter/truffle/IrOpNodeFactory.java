...
///
/// <p>Este mapeamento **NÃO é exaustivo** sobre `IrOp.Kind`: cobre 40 dos 73 `Kind` — as 7
/// categorias da taxonomia da A6 (ALU escalar, multiplicação, memória, transferência múltipla,
/// branch, sistema, ciclo/fetch). Os 33 `Kind` restantes (todo VFP/coprocessador, todo NEON, DSP
/// dual/top-word, `HVC`/`SMC`/`ERET`/`MRS_bank`/`MSR_bank`, bitfield/`RBIT`/`SDIV`/`UDIV`,
...
/// (A10.3 VFP, A10.4 bitfield/divide, A10.5 sistema, A10.6 DSP, A10.7 NEON). Enquanto isso,

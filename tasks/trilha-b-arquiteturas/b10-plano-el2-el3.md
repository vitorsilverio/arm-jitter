...
| **B10.6** | `AT`, formas EL1&0 (`S1E0R`/`S1E0W`/`S1E1R`/`S1E1W`) — escreve `PAR_EL1` com o resultado real de uma tradução (reaproveita `TranslatingAddressSpace64`, regime EL1&0 já existente) | B10.1, B10.2 |
| **B10.6b** | `AT`, formas EL2 (`S1E2R`/`S1E2W`) — exige `TTBR0_EL2` (registrador NOVO, não listado em B10.2) e um segundo regime de tradução stage-1 (estrutura própria de EL2, sem a distinção `AP[1]`/EL0 do regime EL1&0) | B10.2, B10.6 |
| **B10.6c** | `AT`, formas EL3 (`S1E3R`/`S1E3W`) — exige `TTBR0_EL3` E `TCR_EL3` (nenhum dos dois existe ainda; B10.3 não os incluiu) e um terceiro regime de tradução stage-1 | B10.3, B10.6 |
...
| **B10.8** | Stage-2 (`IPA→PA`, `HCR_EL2.VM=1`) — necessário pra `AT S12E1R`/`S12E1W`/`S12E0R`/`S12E0W` valerem de verdade (essas 4 formas ficam pendentes de B10.8, não de B10.6 — combinam stage-1 + stage-2) e pra qualquer guest rodando sob um hypervisor emulado | B10.2, B10.6 |

...
## Onde estamos (atualizado 2026-09-25, B17.5 fechada — tabela de ISA segue em 100%)

**`B17.5` (inteiro SVE sem predicado: aritmética, lógica, shifts, ternárias SVE2, `MLA`/`MLS`/`MAD`/`MSB`, `MOVPRFX`,
`FEXPA`/`FTSSEL`, `INDEX`) fechada** — 34 de 34 encodings, JaCoCo 100% no código novo. `docs/COBERTURA-ISA.md` inalterada
(23523/23523; `sve.decode` só entra na tabela na B17.26). Pendências nomeadas: `FPCR.AH` (FEAT_AFP) no `FTSSEL`; `PTRUE_cnt`/`CNTP_c`
(B17.4). Ver **Resultado** na task.
...
**Pegáveis a seguir** (specs já escritas, dependências satisfeitas): **`B17.6`/`B17.9`/`B17.12`/`B17.13`/`B17.24`** (dependem
de B17.4) e **`B17.8`/`B17.10`** (dependem de B17.5, agora fechada) em diante (SVE, Opção C, VL=256; toda task testa em VL 256 e 512),

...
| ~~P13.7~~ | ~~**G4.2**~~ 🟡 PARCIAL (2026-08-19) — hipótese "framebuffer fica zerado" REFUTADA, nenhum bug real achado na cadeia investigada | `trilha-g-3ds/g4.2-tela-preta-sem-fonte.md` | n3dsemu | G4.1 (hipótese refutada) | dump periódico (mesma técnica de memória byte-a-byte de G2.2/G3.2/G3.3) via `GuestFrameBufferReader.read` — a MESMA função que `Main#presentFrame` usa — mostrou que `hello-world.3dsx` escreve pixels não-zero em `Screen.TOP` a partir de ~slice 180 (endereço `0x14000000`, `stride=480`, `RGB565`) e o conteúdo fica ESTÁVEL (1040/192000 bytes não-zero, ~520 px, primeiro em `col=122,row=1`) por milhares de fatias — sem flicker/corrupção/voltar a zero. A cadeia `GspGpuService`→`FrameBufferState`→`GuestFrameBufferReader` está correta; teste de regressão novo `HelloWorldFramebufferTest` prova isso headless sem GPU. **Não fechada de vez**: se a tela ainda aparecer preta na janela, o candidato restante é downstream (`FrameBufferCodec`/`VulkanRenderer`, upload de textura/rotação retrato→paisagem no shader) — fora do que dá pra provar sem GPU/olho humano (RFC D4), OU o texto é legitimamente pequeno/esparso (~0,5% da tela) e fácil de não notar. `mvn -o test` verde (136). Usuário: rode `n3dsemu testdata/hello-world.3dsx` de novo e diga se aparece uma linha de texto perto do topo da tela — se sim, G4.2 fecha aqui; se realmente nada aparece, a G5 (PICA200/Vulkan) é a próxima suspeita real |
| P14 | **G5** — PICA200 (command list + shader + TEV) | `trilha-g-3ds/g5-pica200-render.md` | n3dsemu | G4.2 | LONGA, 3 PRs; aceite é **só** o `simple_tri`; G4.2 mostrou que o dado de framebuffer 2D já está correto até `GuestFrameBufferReader` — se a tela continuar preta na janela, suspeitar do `VulkanRenderer`/shader antes de a G5 nem começar |

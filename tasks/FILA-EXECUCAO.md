...
| ~~P12.5~~ | ~~**G3.2**~~ 🟡 PARCIAL (2026-08-18) — investigar a inanição do loop de `read-controls.3dsx` (thread de relay do `gsp::Gpu`) | `trilha-g-3ds/g3.2-gsp-relay-starvation.md` | n3dsemu | G3 | causa raiz do sintoma ORIGINAL achada e corrigida (bug real: `RegisterInterruptRelayQueue` devolvia o "GSP module thread index" errado, `1` em vez de `0` — guest lia/escrevia no bloco de fila ERRADO da memória compartilhada); revelou bloqueio NOVO mais profundo (main thread nunca reescalonada depois de criar a thread de relay, mesmo com o relay drenando certo) — candidato a **G3.3**. Ver índice do `tasks/README.md` para o detalhe completo |
| P13 | **G4** — janela Vulkan apresentando os framebuffers | `trilha-g-3ds/g4-vulkan-apresentacao.md` | n3dsemu | G3.3 (ainda não criada) | primeira imagem na tela; NÃO pegar ainda — `read-controls.3dsx` continua sem sair sozinho (achado novo da G3.2, main thread nunca reescalonada) |
...
| P17 | ~~F11~~ 🔴 **BLOQUEADA 2026-08-18** — `virtual-arm-box --machine=raspi3-64` (AArch64, `kernel8.img`) | `trilha-f-infra/f11-raspi3-aarch64-machine.md` | virtual-arm-box | F3 (só M1/M2, não M3) | Sessão 2026-08-18 fechou o que dava pra fechar SEM a máquina completa: `testdata/raspi3-64/` (assets reais + README de proveniência) e `FdtPatcher.withNodeRemoved` (remoção real de nó, 5 testes contra o `.dtb` real). **Parou antes de `Raspi364Machine`**: leitura do código-fonte do `arm-jitter` (não suposição) mostra que B6.6.1-B6.6.5 só cobrem o subconjunto de AArch64 que o corpus de equivalência + `armbox`/`Aarch64LinuxMachine` (ELF64 user-mode) precisam — `Aarch64SystemRegisterId` tem só 9 registradores (nada de `CurrentEL`/`MPIDR_EL1`/`MIDR_EL1`/`ID_AA64*`/`TPIDR_EL1`/timer genérico), `WFI`/`WFE`/`HVC`/`SMC` não decodificam, e `Aarch64Core` não tem NENHUM modelo de IRQ/FIQ. Um kernel EL1 real bateria nessa parede nas primeiras dezenas de instruções — ANTES de qualquer UART/console, sem marco M1-equivalente possível. Movida para a seção 🧑 abaixo (mesma categoria de B6.6.6, mas por lacuna de FEATURE no `arm-jitter`, não por ambiente/toolchain); **destravada por B6.6.7 (P18), priorizada pelo usuário em 2026-08-18** — ver índice do `tasks/README.md` para o relato completo |
| P18 | **B6.6.7** — AArch64: superfície mínima de EL1 para kernel real (system regs restantes + `WFI`/`WFE`/`ERET` + IRQ) | `trilha-b-arquiteturas/b6.6.7-aarch64-el1-kernel-surface.md` | arm-jitter | B6.6.4 | **NOVA, priorizada pelo usuário 2026-08-18** em resposta direta ao bloqueio achado pela F11 (linha acima): estende `Aarch64SystemRegisterId` (`CurrentEL`/`MPIDR_EL1`/`MIDR_EL1`/`ID_AA64*`/`TPIDR_EL1`/timer genérico), decodifica `WFI`/`WFE`/`HVC`/`SMC`, fecha `ERET` se confirmado ausente, e dá a `Aarch64Core` um mecanismo mínimo de entrega de IRQ (espelho de `enterMemoryAbort`, B6.6.4) — GIC/PSCI ficam fora, são responsabilidade do hospedeiro. Destrava F11 (retomar `Raspi364Machine`) **e** B6.6.6 ao mesmo tempo (mesmo bloqueio de fundo, ver linhas na seção 🧑 abaixo) |
...
| ~~B6.6.6~~ **EM ESPERA** (não cancelada) desde 2026-08-18 — hospedeiro `virt64` (kernel arm64 mínimo até shell) | `trilha-b-arquiteturas/b6.6.6-aarch64-virt64-host.md` | Kernel arm64 mainline real (pré-compilado ou toolchain para buildar) + idealmente um initramfs busybox aarch64 real — mesmo bloqueio de toolchain/binário de B6.2 aceite #2/B4.0.3 item 3, um só ambiente resolve os três; adicionalmente, GICv2/GICv3/PSCI/DTB são substancialmente mais complexos que os periféricos versatilepb do precedente B4.1.5, reservar tempo de sessão maior. **Rota alternativa F11 (virtual-arm-box) TENTADA 2026-08-18 e bateu numa parede DIFERENTE e MAIS FUNDA** (ver linha F11 abaixo) — não era sobre GIC/PSCI/toolchain, era sobre o `arm-jitter` ainda não modelar o registrador de sistema/exceção/IRQ que QUALQUER kernel EL1 real (não só `virt`) precisaria. B6.6.6 e F11 agora convergem para o MESMO bloqueio de fundo, **agora atacado pela task B6.6.7 (P18 da tabela de Onda 4, priorizada pelo usuário 2026-08-18)** | fecha o épico B6.6 por completo (depende de B6.6.1-B6.6.5, rodada de spec 2026-07-26, ver `b6-aarch64.md`) — **objetivo prático "Linux arm64 até shell" segue bloqueado até B6.6.7 fechar; ainda precisa de kernel/toolchain real além disso** |
| **F11** — `virtual-arm-box --machine=raspi3-64` (AArch64, `kernel8.img`) | `trilha-f-infra/f11-raspi3-aarch64-machine.md` | **B6.6.7 (P18) fechar** — testdata (`kernel8.img`+DTB reais, com sha256) e `FdtPatcher.withNodeRemoved` (remoção real de nó, pronta para podar `cpu@1-3`) já ficaram prontos na sessão 2026-08-18 e não precisam ser refeitos | destrava `Raspi364Machine`/itens 3-5 do "Inclui" da F11 — e, por tabela, também destrava B6.6.6 (mesmo bloqueio de fundo) |

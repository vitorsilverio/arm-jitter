...
| ~~D5/Buneary~~ 🟡 root cause FECHADA 2026-07-18 (arm-jitter `65a9a66`): bloco JIT stale da troca de overlay — invalidação só olhava o endereço-base da escrita, e um bloco THUMB de 1 instrução em X≡2 (mod 4) escapava da cópia em words; o `b` do overlay 77 (título) executava dentro do overlay 73 e pulava o init da animação. Fix = invalidação por intervalo da escrita; A/B no ROM real confirma (bloco some na troca). **Falta só validação do usuário na GUI** (intro→tutorial→soltar a Buneary, sem F11) | — | 🧑 usuário |

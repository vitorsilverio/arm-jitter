...
| P12 | **G3** — IPC + serviços (`srv:`/`APT`/`hid`/`fs`/`gsp` mínimo) | `trilha-g-3ds/g3-servicos-srv-apt-hid-fs.md` | n3dsemu | G2.2 | `C:\devkitPro\libctru` só tem os headers instalados localmente, **NÃO o código-fonte** — 3dbrew + o próprio `.3dsx` desmontado seguem como oráculo. G2.2 fechada 2026-08-18, boot alcança `svcSendSyncRequest` — pronta para começar |

...
| (sem task, CAUSA IDENTIFICADA 2026-07-22) Platinum Continue = "communication error/Union Room" + tela branca = **FALHA DE INIT DA WIRELESS/WiFi**, NÃO save (melonDS Continua OK com o mesmo .sav; leitura FLASH byte-perfeita; a msg aparece SEM input, ~42 frames, timer de fundo). ARM7 roda firmware wireless de VRAM e lê registradores WiFi 0x04808xxx (0x0480815E lido 252×) que NÃO emulamos → WM expira → erro. Stub 0xFFFF não cura: precisa do handshake WiFi real (BB/RF, W_POWERSTATE...). Fix SUBSTANCIAL (impl. WiFi init, ~centenas de linhas, GBATEK). Detalhes na memória `ndsemu-game-compat` | ndsemu | ⚠️ MODELO FORTE (impl. WiFi) |

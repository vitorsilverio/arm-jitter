...
| P17 | **F11** — `virtual-arm-box --machine=raspi3-64` (AArch64, `kernel8.img`) — retomar `Raspi364Machine` | `trilha-f-infra/f11-raspi3-aarch64-machine.md` | virtual-arm-box | B6.6.7 ✅ | **DESTRAVADA 2026-08-18 pela B6.6.7** (linha acima) — `testdata/raspi3-64/` e `FdtPatcher.withNodeRemoved` já ficaram prontos na sessão de bloqueio anterior (não precisam ser refeitos); a lacuna de feature do `arm-jitter` (identidade de CPU/timer/`WFI`/IRQ) que impedia `Raspi364Machine` de progredir está fechada — próxima sessão pode retomar os itens 3-5 do "Inclui" original da F11 direto |

...
| P14 | **G5** 🟡 PARCIAL (2026-08-19, PR3/3 implementada) — PICA200 (command list + shader + TEV) | `trilha-g-3ds/g5-pica200-render.md` | n3dsemu | G4.2 ✅ | LONGA, 3 PRs; aceite é **só** o `simple_tri` desenhar visualmente, validação do usuário (RFC D4). PR1/PR2: parser+registradores+shader interpretado+`VertexPipeline`, ver `FILA-HISTORICO.md`. **PR3 fechada nesta sessão**: `gpu/tev/TevConfig`+`TevGlslGenerator` (decodifica os 6 estágios TEV dos registradores reais, gera GLSL — testado, mas AINDA não usado pelo pipeline Vulkan, que continua com o shader estático equivalente ao caso `simple_tri`); `gpu/Texture` (formatos não-comprimidos + deswizzle Morton 8×8, testado; não usado pelo Vulkan ainda — task permite adiar p/ PR4); e **o item que faltava para `simple_tri` desenhar de verdade**: `GSPGPU_TriggerCmdReqQueue` agora lê a fila GX real (`gpu/GxCommandQueue`, layout confirmado via 3dbrew), `gpu/shader/ShaderUpload` captura o upload de vertex shader por registrador-FIFO (código/opdescs/uniforms — o caminho real do hardware, não mais só `.shbin`-arquivo), e `GspGpuService` desenha de verdade no `VulkanRenderer` quando um `DrawArrays`/`DrawElements` real dispara (`Main`/`N3dsMachine` injetam o renderer real no modo janela). Teste de integração ponta-a-ponta novo (`GspGpuServiceTest`) monta a fila GX real e confirma 3 vértices corretos chegando ao renderer. `mvn -o test` verde (**180**, +17). Fumaça manual (`--headless`) de `simple_tri.3dsx` não crasha, mesmo padrão de espera de VBlank do `hello-world.3dsx`. **Simplificações documentadas** (Javadoc de `ShaderUpload`): convenção fixa `o0`=posição/`o1`=cor (sem decodificar `SH_OUTMAP`), só uniform float32 (float24 lança exceção), sempre `Screen.TOP`. G5-invariante não se aplica. Detalhe completo em `FILA-HISTORICO.md`. **Falta**: usuário rodar `n3dsemu <path>/simple_tri.3dsx` (janela) e confirmar visualmente; se OK, PR4 (sampler Vulkan de textura + wiring do TEV no pipeline + float24 + `SH_OUTMAP` granular) fecha a trilha G de vez |
| — | **Validação 2026-08-20 (usuário): tela AINDA preta em `simple_tri.3dsx` — bug real diferente do de G4.2, NÃO investigado a fundo** | — | n3dsemu | G5 | Sessão de diagnóstico rápida (fora do orçamento de uma task formal, instrumentação `System.err` temporária, revertida — nada commitado): trace completo de `svcSendSyncRequest`/`svcCreateThread` mostra que o app NUNCA chega a `GSPGPU_TriggerCmdReqQueue`/`WriteHwRegs`/`SetBufferSwap` — trava logo depois de `gsp::Gpu AcquireRight`(0x16)→`RegisterInterruptRelayQueue`(0x13)→cria a thread de relay (handle novo, prioridade 26, mesmo padrão documentado no Javadoc de `GspGpuService`)→`SetLcdForceBlack`(0xB), num loop infinito e estável de `svcArbitrateAddress(r0=3,addr=0x001140CC)`+`svcClearEvent`/`svcWaitSynchronization` alternados no MESMO handle de evento (o `interruptEvent` passado em `RegisterInterruptRelayQueue`) — bate exatamente com o corpo da thread de relay esperando `onVBlank`/`pushInterrupt` sinalizar, mas repetido sem NENHUMA variação por milhares de fatias (`--headless --slices=3000`) e por vários segundos reais em janela — sugere ou que `onVBlank`/`registerPeriodicPulse` não está disparando neste caminho, ou que o `WaitSynchronization` não está bloqueando de verdade (nunca cede a vez pro thread principal continuar o boot). **Não isolado**: não deu tempo de instrumentar `onVBlank`/`Scheduler` em si nesta sessão curta. Diferente do bug da G4.2 (que era só o descriptor set do sampler Vulkan) — aqui o app trava ANTES de emitir qualquer comando de GPU, então nem chega a desenhar nada. Candidata a task nova (**G5.1**?, mesmo formato de investigação de G3.2/G3.3) — próxima sessão deve começar instrumentando `GspGpuService#onVBlank`/`Scheduler#registerPeriodicPulse`/o corpo do `WaitSynchronization` para confirmar qual das duas hipóteses é a real, antes de mexer em código. `mvn -o test` não rodado (nenhum arquivo alterado ao final) |
...
| P17l | **F11** 🟡 PARCIAL (2026-08-20, sessão 6) — retomada após B6.12, achou e fechou o SEXTO gap (bug `Wrapping`, ver E4 acima), SÉTIMO bloqueio real ainda NÃO isolado | `trilha-f-infra/f11-raspi3-aarch64-machine.md` | virtual-arm-box | E4 ✅ | Com o fix da E4 instalado localmente, o boot avança ainda mais longe e um `write64` (`STP`, `executeLoadStorePair`) tenta traduzir para um PA genuinamente acima de 4 GiB (`0x1_0000_0882`) — desta vez uma rejeição correta do `Wrapping` (não é mais o bug do `toIntExact`). **Hipótese forte, NÃO confirmada com instrumentação** (orçamento de tool-calls da sessão já consumido pela investigação do sexto achado): `TranslatingAddressSpace64` só implementa `TTBR0_EL1` (achado documentado desde a spec da `B6.6.3` — "só TTBR0 liga de verdade", mesmo padrão do precedente 32-bit) — Linux real usa `TTBR1_EL1` para o espaço de kernel (VA alto, bit 63 setado); sem TTBR1, o walk usa a tabela ERRADA para qualquer VA de kernel, produzindo descritores lixo cujo PA combinado bate acima de 4 GiB por coincidência. Não é bug, é feature ausente — **fora do "Inclui" desta task**. `Raspi364BootTest` atualizado (`smokeTestBootsWithoutException` não espera mais exceção de `DC IVAC`; `reachesEarlyconBannerInterpreted` volta a `@Disabled`, pinada no novo achado). `mvn -o test` verde no virtual-arm-box. **Sugestão ao usuário**: sub-task nova no arm-jitter (candidata `B6.13`) para suporte a `TTBR1_EL1` em `TranslatingAddressSpace64` (segunda tabela-base + seleção por VA alto/baixo, provavelmente via `TCR_EL1.T1SZ`) + na ponte de registrador de sistema (`Aarch64VmsaSystemRegisters`) — CONFIRMAR a hipótese com instrumentação antes de implementar, mesma disciplina das sub-tasks anteriores |

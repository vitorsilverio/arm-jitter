...
## Onde estamos (atualizado 2026-09-25, B17.6 fechada — tabela de ISA segue em 100%)

**`B17.6` (inteiro SVE predicado: aritmética binária, shifts, unárias; 68 encodings, todos *merging*, `_z` atrás de `SVE2_2`) fechada** —
68 de 68, JaCoCo 100% no código novo, 8 mutações mortas. `docs/COBERTURA-ISA.md` inalterada (23523/23523; `sve.decode` só entra na tabela
na B17.26). Pendência nomeada: `FPCR.AH` (FEAT_AFP) no `FABS`/`FNEG`. Ver **Resultado** na task.
...
**Pegáveis a seguir** (specs já escritas, dependências satisfeitas): **`B17.7`/`B17.9`/`B17.12`/`B17.13`/`B17.24`** (dependem
de B17.4 ou B17.6, ambas fechadas) e **`B17.8`/`B17.10`/`B17.20`** (dependem de B17.5 ou B17.6) em diante (SVE, Opção C, VL=256; toda task testa em VL 256 e 512),

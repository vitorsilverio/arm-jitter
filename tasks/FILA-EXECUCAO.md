...
| (sem task, ATUALIZADA 2026-07-19) Platinum GAMEPLAY: personagem INVISÍVEL (transparente) no quarto — chars = billboards 3D f3 32×32 lendo slot0/0x78A0+0xB5E0, mas esses endereços ficam VAZIOS: o upload da textura do char (slot0) NUNCA acontece (bank A só recebe texturas de mapa até 0x763C e para; bank B streama p/ slot1 que ninguém lê). Falta um TRANSFER inteiro (provável VRAM transfer manager NitroSDK `RAM_21c07c4`). Fix de fase `6f72757` correto mas não basta. **Precisa oráculo melonDS** — detalhes/funções na memória `ndsemu-game-compat` | ndsemu | ⚠️ MODELO FORTE |
| (sem task, INVESTIGADA 2026-07-19) Platinum Continue = "communication error/Union Room" (msg archive 200) + tela branca (ARM9 eterno em OS_WaitIrq após o A). DESCARTADO: save íntegro (CRCs ok), leitura FLASH byte-perfeita, sem falha IPC/som/wireless ao vivo, slot vazio irrelevante. Decisão do erro no overlay do estado Continue (0x0201C2B8), não isolada. **ÚNICO próximo passo: carregar o MESMO .sav no melonDS + Continue** — separa "nossos bytes de save errados (save-WRITE)" de "diferença do nosso emu no Continue". Repro `ContinueProbe.java`. Detalhes na memória `ndsemu-game-compat` | ndsemu | ⚠️ MODELO FORTE + oráculo melonDS |

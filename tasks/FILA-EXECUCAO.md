...
| P17e | **F11** — retomar `Raspi364Machine` (sessão 4), destravada pela B6.9 | `trilha-f-infra/f11-raspi3-aarch64-machine.md` | virtual-arm-box | B6.9 ✅ | próxima sessão pickável: rodar o boot do `kernel8.img` real de novo com `AND`/`ORR`/`EOR`/`ANDS`/`BIC`/`ORN`/`EON`/`BICS`/`MOV`/`MVN` disponíveis (precisa `mvn -o install` do arm-jitter, já feito nesta sessão) — esperar mais gaps de decode na mesma classe "Data Processing — Register" antes de presumir boot completo, mesmo padrão iterativo de B6.8→B6.9 |

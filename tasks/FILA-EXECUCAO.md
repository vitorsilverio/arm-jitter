...
| P11.5 | **G2.2** — investigar o laço de `svcCreateAddressArbiter` (novo, criado 2026-08-18 a pedido do usuário) | `trilha-g-3ds/g2.2-address-arbiter-loop.md` | n3dsemu | G2 | task de INVESTIGAÇÃO — isolar por que o guest chama `svcCreateAddressArbiter` sempre no mesmo PC sem avançar; **destrava G3**. Ver a nota logo abaixo desta tabela para o contexto completo |
| P12 | **G3** — IPC + serviços (`srv:`/`APT`/`hid`/`fs`/`gsp` mínimo) | `trilha-g-3ds/g3-servicos-srv-apt-hid-fs.md` | n3dsemu | G2.2 | `C:\devkitPro\libctru` só tem os headers instalados localmente, **NÃO o código-fonte** — 3dbrew + o próprio `.3dsx` desmontado seguem como oráculo. Bloqueada em G2.2, não em G2 diretamente (ver acima) |

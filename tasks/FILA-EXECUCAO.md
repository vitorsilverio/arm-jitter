...
## ✅ B10 — EL2/EL3 completo — FECHADA 2026-08-22 (só `B10.6b`/`B10.6c` seguem fora, sem consumidor
## real que os force hoje — bloqueadas em `TTBR0_EL2`/`TTBR0_EL3` novos, não implementados por
## nenhuma task desta escada). Volta ao usuário decidir o próximo item da fila (Onda 5 restante,
## Q5+, ou outra prioridade) — nada aqui deve ser pego automaticamente sem essa decisão.
...
| R6b | **B10.6b** — `AT`, formas EL2 (`S1E2R`/`S1E2W`) | `b10-plano-el2-el3.md` | arm-jitter | B10.2, B10.6 | exige `TTBR0_EL2` NOVO (não existe) + segundo regime de tradução; sem task spec ainda |
| R6c | **B10.6c** — `AT`, formas EL3 (`S1E3R`/`S1E3W`) | `b10-plano-el2-el3.md` | arm-jitter | B10.3, B10.6 | exige `TTBR0_EL3`+`TCR_EL3` NOVOS (nenhum existe) + terceiro regime de tradução; sem task spec ainda |
...
**🔔 Próximo passo recomendado ao usuário: publicar `arm-jitter` `1.1.0` no Maven Central** (marco
cruzado pela B8.5 — global 53%→59% desde o `1.0.0` — e REFORÇADO pela B9.7: v7-A 83%→95%, **+12pp**)
seguindo o procedimento da `F5` (`trilha-f-infra/f5-maven-central-publicacao.md`), depois **F7** nos
4 consumidores. Não pego automaticamente por ser publicação externa/irreversível.

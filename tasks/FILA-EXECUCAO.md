...
| P14 | **G5** 🟡 PARCIAL (2026-08-19, PR2/3) — PICA200 (command list + shader + TEV) | `trilha-g-3ds/g5-pica200-render.md` | n3dsemu | G4.2 ✅ | LONGA, 3 PRs; aceite é **só** o `simple_tri`. PR1 (`bae9813`): parser+registradores, ver histórico. **PR2 fechado nesta sessão**: `gpu/shader/ShaderBinary`+`VertexShaderInterpreter` (ISA formatos 1/1u, ~17 opcodes) + `gpu/VertexAttributeLoader` (algoritmo de `Pica::VertexLoader` do Citra real) + `gpu/VertexPipeline` + `PicaRenderer#drawTriangles`/`ShadedVertex` (implementado em `RecordingRenderer` e `VulkanRenderer` — *render pass*/pipeline de geometria novo, desenha na `ScreenTexture` já existente da G4). **Validação incomum**: tudo cross-validado campo a campo contra um `.shbin` REAL compilado pelo `picasso.exe` a partir do `.v.pica` de origem do `simple_tri` (`testdata/shaders/simple_tri.shbin`) — não só transcrição do 3dbrew; achou a base de registrador de constantes da ISA (32) ser diferente da base das tabelas de metadados do DVLE (16). **Escopo cortado conscientemente** (documentado no Javadoc): CMP/MAD/controle de fluxo NÃO implementados (bit-layout inconsistente entre buscas no 3dbrew, sem `.shbin` real pra cross-validar, nenhum exemplo do M5 usa) — lançam `UnsupportedOperationException`. **Também não integrado** ao comando-list ao vivo/boot real de `simple_tri.3dsx` ainda (permutação de atributos/upload de shader via FIFO da GPU real) — fica para quando a PR3 fechar a cor final via TEV. `mvn -o test` verde (163, +17); `VulkanRendererSmokeTest` estendido roda o pipeline de geometria contra o driver Vulkan REAL desta máquina com validation layers (achou 1 bug de teste: tela nunca populada fica `UNDEFINED`, não afeta o `Main` real). G5-invariante não se aplica (nenhum arquivo arm-jitter tocado). **Falta**: PR3 (texturas + TEV — fecha a cor final e integra ao boot real) — próxima sessão pega PR3 |

...
| P14 | **G5** 🟡 PARCIAL (2026-08-19, PR1/3) — PICA200 (command list + shader + TEV) | `trilha-g-3ds/g5-pica200-render.md` | n3dsemu | G4.2 ✅ | LONGA, 3 PRs; aceite é **só** o `simple_tri`. **PR1 fechado** (`bae9813`): `gpu/CommandListParser` (decodifica pares parâmetro+cabeçalho — máscara de byte, contador de palavras extras, modo consecutivo, conforme 3dbrew GPU/Internal Registers) + `gpu/PicaRegisters` (banco de 0x1000 registradores internos com escrita mascarada real de hardware + contadores de `FINALIZE`/`DRAW_ARRAYS`/`DRAW_ELEMENTS`). **Achado real**: o texto da própria task diz "0x0010=trigger draw, 0x00E0/0x00E1=finalize" — o 3dbrew (fonte que a task manda transcrever) diz o oposto (`0x0010=GPUREG_FINALIZE`, `0x022E/0x022F=DRAWARRAYS/DRAWELEMENTS`, sem `0x00E0/0x00E1` documentado); implementado conforme o 3dbrew, não a paráfrase da task. Views tipadas de vértice/TEV/texturas ficam para PR2/PR3 (sem consumidor ainda). `mvn -o test` verde (146, +10 novos); G5-invariante (gate de regressão) não se aplica, nenhum arquivo arm-jitter tocado. **Falta**: PR2 (vertex shader interpretado + geometria Vulkan) e PR3 (texturas + TEV) — próxima sessão pega PR2 |

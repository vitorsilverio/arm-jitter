...
| P12.6 | **G3.3** — investigar por que a thread principal de `read-controls.3dsx` nunca é reescalonada | `trilha-g-3ds/g3.3-main-thread-never-rescheduled.md` | n3dsemu | G3.2 | 🟡 PARCIAL (2026-08-18, spec escrita e investigada na mesma sessão) — causa raiz NÃO é o `Scheduler` (confirmado com trace instrumentado, revertido/não commitado): `T1` bloqueia para sempre em `svcArbitrateAddress` esperando o endereço `0x0011D26C`, que nenhum código sinaliza. Hipótese forte (fonte real do libctru via `WebFetch`): é um `GSPGPU_Event` que este HLE nunca gera (só gera VBlank da tela de cima, RFC D6). Aritmética exata de offset não fechou — retomar com o `LR` do chamador (não só o PC do wrapper de `svc`), ver a task para o passo a passo completo de reprodução |
| P13 | **G4** — janela Vulkan apresentando os framebuffers | `trilha-g-3ds/g4-vulkan-apresentacao.md` | n3dsemu | G3.3 (PARCIAL) | primeira imagem na tela; NÃO pegar ainda — G3.3 achou a causa raiz mas não fechou o fix, `read-controls.3dsx` continua sem sair sozinho |

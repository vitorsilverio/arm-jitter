...
| [B1.6](trilha-b-arquiteturas/b1.6-armv6-asm-nativo.md) | Emissão nativa ASM das ops v6 | B1.2–B1.5 | 🟡 PR1/3 (extend/reverse/UMAAL, B1.2) ✅ + PR2/3 (paralelas+GE+PKH+saturação+USAD, B1.3) ✅ — SXT*/UXT*/REV*/UMAAL agora nativos: bytecode direto (I2B/I2S/AND/`Integer.reverseBytes`) + helpers `extendByte16`/`reverseHalfwords`/`reverseSignedHalfword`; UMAAL soma RdLo/RdHi como duas parcelas u32 no `LMUL` já emitido. ParallelAlu/Sel/Saturate/AbsDiffSum via helpers `parallelAlu`/`sel`/`saturate`/`absDiffSum` em `AsmRuntimeHelpers` (GE e Q sticky como efeito colateral no core, espelhando o padrão ARMv5TE); PKHBT/PKHTB viram bytecode direto (AND+OR). `AsmNativePolicy` só rejeita mais LDREX/STREX/CLREX (B1.4) e as instruções de sistema B1.5 — `supportedAluOpcodes()` agora é `EnumSet.allOf`. Equivalência exaustiva 14 cond × 16 NZCV por op (`ArmV6ExtendReverseNativeEquivalenceTest` + `ArmV6ParallelSaturateNativeEquivalenceTest`) + `perOpFallbackOpCount()==0` nos blocos sintéticos. Suites arm-jitter 492 + gbaemu 216 + ndsemu 175 verdes. Falta PR3 (LDREX/STREX/CLREX, B1.4) |

...
| [B1.6](trilha-b-arquiteturas/b1.6-armv6-asm-nativo.md) | Emissão nativa ASM das ops v6 | B1.2–B1.5 | 🟡 PR1/3 (extend/reverse/UMAAL, B1.2) ✅ — SXT*/UXT*/REV*/UMAAL agora nativos: bytecode direto (I2B/I2S/AND/`Integer.reverseBytes`) + helpers `extendByte16`/`reverseHalfwords`/`reverseSignedHalfword` em `AsmRuntimeHelpers`; UMAAL soma RdLo/RdHi como duas parcelas u32 no `LMUL` já emitido; `AsmNativePolicy` só rejeita mais PKHBT/PKHTB (B1.3) nas ALU v6. Equivalência exaustiva 14 cond × 16 NZCV por op (`ArmV6ExtendReverseNativeEquivalenceTest`) + `perOpFallbackOpCount()==0` num bloco síntetico só de v6. Suites arm-jitter 486 + gbaemu 216 + ndsemu 175 verdes. Faltam PR2 (paralelas+GE+saturação, B1.3) e PR3 (LDREX/STREX/CLREX, B1.4) |

...
/// for both A32 and T32"*), 23 linhas ao todo. Esta task (B13.17) implementa as **4** de `VCMLA`/
/// `VCADD`/`VCMLA_scalar` (`FEAT_FCMA`); as outras 19 (`VSDOT`/`VUDOT`/`VUSDOT`/`VDOT_b16`/`VFML`/
/// `VSMMLA`/`VUMMLA`/`VUSMMLA`/`VMMLA_b16`/`VFMA_b16` + as formas `_scalar`/`_scal` correspondentes,
/// B13.18-B13.21) ainda não têm dono.
///
/// **Este decoder devolve `null` (não `unimplemented`) para o que ainda não tem dono** — decisão
/// registrada na task: como o arquivo cresce ao longo de 5 tasks (B13.17-B13.21), reivindicar o
/// frame inteiro agora faria as 19 linhas restantes virarem `UNIMPLEMENTED` prematuramente, antes
...
/// `UNIMPLEMENTED` no fim da cadeia de extensões — o comportamento observável para as 19 linhas
/// não muda.
///
...
/// bit7:bits[19:16], `Vm` = bit5:bits[3:0] (exceto na forma indexada `size=1`, onde `Vm` é um
/// nibble DIRETO em bits[3:0], sem bit de extensão — ver {@link #decodeComplexScalar}).
///
/// Encodings golden conferidos com `arm-none-eabi-as -march=armv8.3-a -fpu neon-fp-armv8
/// -mfpu=neon-fp-armv8 .arch_extension fp16` (devkitARM) — ver `## Resultado` da task para o log.
///
/// Gate: {@link ArmFeature#COMPLEX_NUMBER_ARITHMETIC} (`FEAT_FCMA`). **Nenhum preset a declara**
/// (a saída de `NOT_IN_ANY_PRESET` é a B13.22), então sem a feature todo o frame cai no
...
public final class NeonSharedDecoder implements DecoderExtension {
    ...
    public DecodedInstruction tryDecode(int raw, int address, Condition condition) {
        ...
        // Ainda sem dono (B13.18-B13.21): devolve `null` de propósito, ver javadoc da classe.
        return null;
    }

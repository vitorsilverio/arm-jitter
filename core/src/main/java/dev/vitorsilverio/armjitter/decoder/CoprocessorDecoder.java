...
/// Decodifica o espaço de transferência DUPLA de registrador de coprocessador ARM (`MCRR`/`MRRC`,
/// ARMv5TE/extensão "E") que ARMv5 adiciona acima da base compartilhada ARMv4T, delegando a
/// {@link CoprocessorRegisterDecoder} para a transferência SIMPLES (`MCR`/`MRC`, ARMv3+, já
/// aplicável ao próprio `ARMV4T` desde a B9.13). Anexado à
/// {@link dev.vitorsilverio.armjitter.arch.ArmArchitecture#ARMV5TE}, para que apenas cores ARMv5+
/// (o ARM9 do NDS, o ARM11 do raspi1/F3) decodifiquem `MCRR`/`MRRC`; o decoder base não é alterado.
///
/// `MCRR`/`MRRC` (F3, `virtual-arm-box` raspi1) — codificação: `cccc 1100 010 L RRRR tttt pppp
/// oooo MMMM` — bits 27-21 = 1100010, distinto do espaço `MCR`/`MRC` (bits 27-25 = 111 vs
...
public final class CoprocessorDecoder implements DecoderExtension {
    ...
    private final CoprocessorRegisterDecoder singleTransfer = new CoprocessorRegisterDecoder();
    ...
    public DecodedInstruction tryDecode(int raw, int address, Condition condition) {
        ...
        return singleTransfer.tryDecode(raw, address, condition);
    }
}

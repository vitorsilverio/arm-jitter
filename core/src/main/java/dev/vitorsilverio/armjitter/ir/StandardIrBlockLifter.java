    ...
    private boolean isTerminal(DecodedInstruction instruction) {
        ...
        return switch (instruction.kind()) {
            ...
            case BRANCH, BRANCH_EXCHANGE, LONG_BRANCH_SUFFIX, LONG_BRANCH_32, POP, SWI, HVC, SMC, ERET,
                    ...
                    // HALT (B22.1, `HLT`): mesma categoria de BKPT — nenhum preset a executa hoje
                    // (recusada no decode por falta de `ArmFeature.HALT`), mas terminal por
                    // definição (para o core / entrega ao debugger do host).
                    BREAKPOINT, HALT,
                    // NOCP (perfil M, B15.2): sempre troca o PC (entra em USAGE_FAULT via
                    // MProfileExceptionModel), mesma categoria de HVC/SMC/BREAKPOINT acima.
                    NOCP,
                    // SECURE_GATEWAY (SG, B15.4): não troca o PC, mas muda o estado de segurança
                    // observável (Secure/Non-secure) e o banco de SP ativo — terminal pela mesma
            ...
            case MOV, ADD, ADC, SUB, RSB, SBC, RSC, NEG, AND, EOR, ORR, LSL, LSR, ASR, ROR, MUL, MLA, UMULL, UMLAL, SMULL, SMLAL, CLZ, SATURATING, DSP_MULTIPLY, DSP_DUAL_MULTIPLY, DSP_TOP_WORD_MULTIPLY, EXTEND, BYTE_REVERSE, UMAAL, PARALLEL_ALU, SEL, PKH, SATURATE, USAD8, LOAD_EXCLUSIVE, STORE_EXCLUSIVE, CLEAR_EXCLUSIVE, BIC, MVN, MRS, MSR, TST, TEQ, CMP, CMN, LOAD_LITERAL, LOAD, STORE, DOUBLE_TRANSFER, SWAP, LOAD_MULTIPLE, STORE_MULTIPLE, LONG_BRANCH_PREFIX, PUSH,
                    ...
                    // VFP_MOVE_HALF_LANE (B14.6): `VMOVX`/`VINS` idem — troca crua de metades de
                    // um `S`, nunca escreve PC.
                    VFP_ALU, VFP_MOVE_IMMEDIATE, VFP_COMPARE, VFP_CONVERT, VFP_SELECT, VFP_ROUND,
                    VFP_CONVERT_ROUNDED, VFP_MOVE_HALF_LANE, VFP_LOAD, VFP_STORE,
                    // B14.6b (`_hp`): idem — nenhuma toca o PC.
                    ...
                    // recusado no decode, UNPREDICTABLE — destino/origem é FPSCR, não um GPR).
                    VFP_SYSREG_LOAD, VFP_SYSREG_STORE,

...
| `ARMV8A_32` (ARMv8-A AArch32-only, `Cortex-A32`) | ✅ decode+interpretado (B14.1-B14.6: `LDA`/`STL`+exclusivos, `CRC32*`, `VSEL`/`VMAXNM`/`VMINNM`, `VRINT{A,N,P,M}`/`VCVT{A,N,P,M}`, `VMOVX`/`VINS`, aritmética `_hp`; `docs/COBERTURA-ISA.md` coluna `v8-A/32` medida pela **B14.7**, `vfp-uncond.decode` **17/17**, preset geral **96%** (749/776) depois da **B22.7**: `VRINTR`/`VRINTZ`/`VRINTX` (`sp`/`dp`/`hp`, `FPSCR.RMode` corrente), `VCVTR`, `VCVTB`/`VCVTT` f16↔f32/f64 e `VJCVT`/`VCVT_b16_f32` sob o preset `ARMV8_6A_32` (coluna `v8.6-A/32`, 751/778) — os `❌` restantes desta coluna não são VFP) + catálogo `ArmProcessor.CORTEX_A32` (B14.7, resolve a pendência B12.6) | ⬜ (nenhum torture dedicado ainda — herda `armv7a-torture` na parte comum) | ⬜ | ⬜ | candidato a task própria: torture/binário real de `Cortex-A32`; N4 = nenhum consumidor real ainda. os `❌` expostos pela B14.7 (`VRINTR*`/`VRINTZ*`/`VRINTX*`, `VJCVT`, conversões `VCVT_f32_f16`&cia) foram fechados pela B22.7 (ver `## Resultado` dela; `FJCVTZS` do A64 também foi corrigido lá — reduzia a 0 em vez de módulo 2³²) |

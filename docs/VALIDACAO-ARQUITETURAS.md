...
| `ARMV6K` | ✅ (B1.1-B1.6) | ✅ armv6k-torture (B4.0.1) | 🟡 só hello-armv6k (sinal fraco: sem instrução v6 de compilador) | ✅ MPCore (3DS/Pi1, B5.2, preset `ARM11_MPCORE`, sem emulador hospedeiro ainda) | busybox/gcc armv6k real — entra de carona no linuxbox (B4.1.5, kernel versatile é v6) |
...
| MMU/full-system (ARMv6 VMSA) | ✅ (B4.1.1-B4.1.4: `TranslatingAddressSpace`, `Cp15VmsaCoprocessor`, aborts precisos nos 3 motores, `translationGeneration`) | ✅ `MemoryAbortEquivalenceTest`/`ArmCoreMemoryAbortTest` (B4.1.3) | 🟡 kernel Debian real (`vmlinuz-3.2.0-4-versatile`) + busybox-armv5l descomprimem no `linuxbox` (B4.1.5), mas travam num `PREFETCH_ABORT` recursivo antes do shell — causa raiz não isolada | ⬜ | épico B4.1 quase fechado — retomar o loop de abort do `linuxbox` (B4.1.5), ou fechar com toolchain `arm-linux-gnueabihf-*`/WSL real e kernel buildado do zero |

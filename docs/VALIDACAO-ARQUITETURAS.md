...
| `ARMV7M_PURE`/`ARMV8M_BASELINE`/`ARMV8M_MAINLINE`/`ARMV8_1M` (perfil M moderno) | ✅ (B15.1-B15.6: `NOCP`/`VMSR_VMRS`/`VLDR_sysreg`/`VSTR_sysreg`/`VLLDM`/`VLSTM`/`VSCCLRM`/`SG`/`BXNS`/`BLXNS`/`TT`/`DLS`/`WLS`/`LE`, `m-nocp.decode`=11/11 confirmado pela B15.7) + catálogo `ArmProcessor` fechado (B15.7: `SC300`/`Cortex-M3`/`M4`/`M7`/`M23`/`M33`/`M35P` completos, `M52`/`M55`/`M85` parciais — sem Helium/MVE, épico B16 `⬜`) | ⬜ (nenhum torture dedicado ainda — herda `cortexm-torture` m0/m3 nos presets que compartilham base) | ⬜ | ⬜ | candidato a task própria: torture/binário real de Cortex-M23/M33 (Security Extension) e Cortex-M52/M55/M85 (parte escalar) — bloqueado pelo mesmo desvio de toolchain de B6.2/B6.6.6/F11; N4 = nenhum consumidor real ainda |

...
| `ARMV8A_32` (ARMv8-A AArch32-only, `Cortex-A32`) | ✅ decode+interpretado (B14.1-B14.6: `LDA`/`STL`+exclusivos, `CRC32*`, `VSEL`/`VMAXNM`/`VMINNM`, `VRINT{A,N,P,M}`/`VCVT{A,N,P,M}`, `VMOVX`/`VINS`, aritmética `_hp`; `docs/COBERTURA-ISA.md` coluna `v8-A/32` medida pela **B14.7**, `vfp-uncond.decode` **17/17**, preset geral **94%** (736/778) — os `❌` restantes são `VRINTR/Z/X`, `VJCVT`, conversões FP16 do VFPv3, degraus posteriores já nomeados) + catálogo `ArmProcessor.CORTEX_A32` (B14.7, resolve a pendência B12.6) | ⬜ (nenhum torture dedicado ainda — herda `armv7a-torture` na parte comum) | ⬜ | ⬜ | candidato a task própria: torture/binário real de `Cortex-A32`; N4 = nenhum consumidor real ainda. `❌` expostos pela B14.7 (`VRINTR*`/`VRINTZ*`/`VRINTX*`, `VJCVT`, conversões `VCVT_f32_f16`&cia) viram tasks próprias, ver `## Resultado` da B14.7 |
...
| `ARMV7M_PURE`/`ARMV8M_BASELINE`/`ARMV8M_MAINLINE`/`ARMV8_1M`/`ARMV8_1M_MVE` (perfil M moderno) | ✅ (B15.1-B15.6: `NOCP`/`VMSR_VMRS`/`VLDR_sysreg`/`VSTR_sysreg`/`VLLDM`/`VLSTM`/`VSCCLRM`/`SG`/`BXNS`/`BLXNS`/`TT`/`DLS`/`WLS`/`LE`, `m-nocp.decode`=11/11 confirmado pela B15.7; **B16.2-B16.13 fecharam decode+execução interpretada de Helium/MVE**, `mve.decode`=352/352 medido pela **B16.14** contra `ARMV8_1M_MVE`, sem `⚠️`) + catálogo `ArmProcessor` fechado (B15.7: `SC300`/`Cortex-M3`/`M4`/`M7`/`M23`/`M33`/`M35P` completos; B16.14: `M52`/`M55`/`M85` agora resolvem para `ARMV8_1M_MVE`, MVE-I+MVE-F completos — ASM nativo/Truffle de MVE seguem pendentes, épicos **C12**/**A10**) | ⬜ (nenhum torture dedicado ainda — herda `cortexm-torture` m0/m3 nos presets que compartilham base) | ⬜ | ⬜ | candidato a task própria: torture/binário real de Cortex-M23/M33 (Security Extension) e Cortex-M52/M55/M85 (Helium/MVE) — bloqueado pelo mesmo desvio de toolchain de B6.2/B6.6.6/F11; N4 = nenhum consumidor real ainda. B16.14 achou 34 gaps residuais no espaço T32 sob `ARMV8_1M_MVE` (long-shift GPR-pair MVE, `BF` `BFL`/`BFCSEL`/`BFX`/`BFLX`, tail-predication `WLSTP`/`DLSTP`/`LCTP`/`VCTP`, `CLRM`, `SB`/`CRC32*`) — ver `## Resultado` da B16.14 |
...
| AArch64 | ✅ (B6.1-B6.5 base + B6.6.x MMU/EL1, B6.8-B6.14, escada B8 AdvSIMD completa, B10 EL2/EL3, B11 gating por versão/feature nos 16 presets `ARMv8.0-A`…`ARMv9.5-A`, B12 catálogo de processadores, **épico B19 FECHADO (B19.1-B19.13, B19.9 2026-09-10)**; decode+interpretado medido em `docs/COBERTURA-ISA.md`: `ARMv8.0-A` **99%** (858/862, era 82%/174 `❌` no início do épico em 2026-08-29), `ARMv9.5-A` **90%** (1033/1146). O que resta no A64 já tem destino nomeado — ver `## Resultado` da B19.9 e `b19-plano-a64-gap-remanescente.md`, nenhum grupo sem degrau) | — (sem torture A64 dedicado ainda; corpus real via `aarch64-none-elf-as`/`objdump` + devkitA64 cobre decode instrução a instrução) | 🟡 hello-aarch64.elf bare-metal real (devkitA64, B6.2 aceite #1) ✅; busybox aarch64 real (B6.2 aceite #2) NÃO fechado — só existe `armv8l` (ISA errada) no busybox.net e o devkitA64 é bare-metal | ⬜ | **B19.14-B19.29** (grupos remanescentes por feature, ver B19.9) + **B6.6.6** (hospedeiro `virt64`: kernel arm64 real + GICv2/v3/PSCI/DTB, bloqueado no usuário pelo mesmo desvio de toolchain do B6.2 aceite #2) |
...
> **17/17** no grupo `vfp-uncond.decode`, **94%** (736/778) no preset inteiro — os `❌`
> restantes (`VRINTR*`/`VRINTZ*`/`VRINTX*`/`VJCVT`/conversões FP16 do VFPv3) são degraus
> cronológicos POSTERIORES a `ARMv8.0-A`, já nomeados como pendência, não regressão. E o A64

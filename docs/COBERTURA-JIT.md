...
> **ASM 64 bits: 46 de 126** `Kind` emitidos nativamente.
> **Truffle 64 bits: 0 de 126** — o backend não existe (A10.8).
...
Linhas = os 126 `Ir64Op.Kind`. `Ir64NativePolicy` casa por `Kind` e **não tem carve-outs condicionais** (sem `⚠️` deste lado). A coluna Truffle é inteira `❌` (A10.8).

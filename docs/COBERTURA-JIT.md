...
> **ASM 64 bits: 46 de 127** `Kind` emitidos nativamente.
> **Truffle 64 bits: 0 de 127** — o backend não existe (A10.8).
...
Linhas = os 127 `Ir64Op.Kind`. `Ir64NativePolicy` casa por `Kind` e **não tem carve-outs condicionais** (sem `⚠️` deste lado). A coluna Truffle é inteira `❌` (A10.8).
